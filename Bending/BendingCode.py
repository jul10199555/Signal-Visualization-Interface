...
def go_home(rpm_busqueda=VELOCIDAD_MEDICION):
    ...
    if hall0_activo():
        # Liberar
        small_release = max(5, estimar_pulsos_entre_sensores() // 10)  # ~10% del tramo
        base = abs(pulse_count)
    ...
    # 2) Buscar Hall 0 y detener
    timeout = int(FACTOR_TIMEOUT_BUSQUEDA * max(estimAR := estimar_pulsos_entre_sensores(), 50))
...
def mode1_action(cfg):
    try:
        ...
        global current_direction, velocidad_constante, angulo_constante, pulsos_objetivo,direccion_aumentando_velocidad, direccion_aumentando_angulo
