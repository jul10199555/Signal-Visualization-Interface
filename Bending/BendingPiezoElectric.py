...
# Protocol: PC -> "0\n" | Pico -> "0\n" | luego imprime "READY\n"
# Versión: CON ADS1115 para sensor PVDF 
# Fecha: 2026-05-19
# ============================================================
...
# ============================================================
#   ADS1115 PARA SENSOR PVDF (GP20=SDA, GP21=SCL)
# ============================================================
i2c_ads = I2C(0, sda=Pin(20), scl=Pin(21), freq=400000)
ultimo_voltaje_pvdf_mv = 0.0

def leer_pvdf():
    """Lee el voltaje del sensor PVDF desde el ADS1115 (canal A0)"""
    global ultimo_voltaje_pvdf_mv
    
    try:
        dispositivos = i2c_ads.scan()
        if 0x48 not in dispositivos:
            return ultimo_voltaje_pvdf_mv
        
        # Configurar conversión: canal A0, ±4.096V, single-shot
        config = 0x8000 | (0 << 12) | 0x0200 | 0x0003
        i2c_ads.writeto_mem(0x48, 0x01, config.to_bytes(2, 'big'))
        time.sleep_ms(30)
        
        data = i2c_ads.readfrom_mem(0x48, 0x00, 2)
        valor = int.from_bytes(data, 'big')
        if valor & 0x8000:
            valor = valor - 65536
        
        voltage_mv = valor * 0.125
        ultimo_voltaje_pvdf_mv = voltage_mv
        return voltage_mv
        
    except Exception as e:
        dbg("Error leyendo PVDF:", e)
        return ultimo_voltaje_pvdf_mv
...
def medir_angulo_entre_sensores(timeout_ms=HALL_WAIT_MAX_MS_DEFAULT):
    ...
    while hall_sensor_90.value() == 1:
        ...
        if time.ticks_diff(time.ticks_ms(), t0) > timeout_ms:
            stop_motor()
            raise RuntimeError("Timeout esperando Hall90")
        time.sleep(0.001)
    stop_motor()
    time.sleep(0.2)
    pulse_reset_atomic()
    ...
    while hall_sensor_0_a.value() == 1:
        ...
        if time.ticks_diff(time.ticks_ms(), t1) > timeout_ms:
            stop_motor()
            raise RuntimeError("Timeout esperando Hall0")
        time.sleep(0.001)
    stop_motor()
    gpp = (GRADOS_POR_PULSO_FORWARD + GRADOS_POR_PULSO_BACKWARD) / 2
    if gpp <= 0:
        gpp = 0.014
    return abs(pulse_get_atomic()) * gpp
...
def calibrar_motor():
    ...
    for i in range(ciclos + 1):
        ...
        while sensor_siguiente.value() == 1:
            ...
            if time.ticks_diff(now, t0) > CALIB_STEP_MAX_MS:
                stop_motor()
                _led_all_off()
                raise RuntimeError("Timeout en calibrar_motor()")
            time.sleep(0.001)
        time.sleep(0.02)
        stop_motor()
        if i > 0:
    ...
    if not forward_pulses or not backward_pulses:
        raise RuntimeError("Calibración inválida")
...
# ============================================================
#   MODOS 2, 3, 4 (CON LECTURA DE PVDF)
# ============================================================
...
def mode2_action(cfg):
    ...
    voltaje_pvdf = leer_pvdf()
    sys.stdout.write(str([
        "modo", 2, kia, ia, kfa, fa, ksa, sa, kv, v,
...
def main():
    ...
    try:
        dispositivos = i2c_ads.scan()
        if 0x48 in dispositivos:
            dbg("✅ ADS1115 detectado en 0x48")
        else:
            dbg("⚠️ ADS1115 NO detectado")
    except Exception as e:
        dbg("Error detectando ADS1115:", e)
    ...
    while True:
        ...
        if line:
            ...
            if t_upper.startswith("GOTO"):
                ...
                if angle is not None:
                    ...
                    sys.stdout.write("ERROR: formato GOTO inválido\n")
                continue
        ...
        # Envío continuo del PVDF en IDLE
        if state == STATE_IDLE:
            now = time.ticks_ms()
            if time.ticks_diff(now, next_pvdf_t) >= INTERVAL_MS:
                voltaje_pvdf = leer_pvdf()
                sys.stdout.write(str(["pvdf_mv", voltaje_pvdf]) + "\n")
                next_pvdf_t = time.ticks_add(now, INTERVAL_MS)
        ...
        if state == STATE_IDLE:
            ...
            try:
                ...
                if modo not in MODE_HANDLERS:
                    sys.stdout.write("ERROR: modo 1..4\n")
                    continue
                if modo == 2:
                    mode2_state = 0
                    mode2_rep_count = 0
